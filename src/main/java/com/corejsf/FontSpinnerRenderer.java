...
@FacesRenderer(componentFamily = "javax.faces.Input", rendererType = "com.corejsf.FontSpinner")
...
public class FontSpinnerRenderer extends Renderer {
	...
	public void encodeBegin(FacesContext context, UIComponent component)
		...
		if (min != null) {
			writer.write(MessageFormat.format(
					"document.forms[''{0}''][''{1}''].min = {2};", formId,
					clientId, min));
		}

		if (max != null) {
			writer.write(MessageFormat.format(
					"document.forms[''{0}''][''{1}''].max = {2};", formId,
					clientId, max));
		}
		...
		// /
		String spinScript = MessageFormat
				.format("com.corejsf.spinner.spin(document.forms[''{0}''][''{1}''], -1);",
						formId, clientId);
	...
	private Object getChangeScript(FacesContext context, UIInput component) {
